...
public class UserDAO {
    ...
    private final String SQL_LIST_USERS =
            "SELECT email, password, name, function, phone, communityName FROM user";
    private final String SQL_GET_USER_BY_LOGIN =
            "SELECT email, password, name, function, phone, communityName FROM user WHERE email = ?";
    private final String SQL_EXISTS_USER = "SELECT 1 FROM user WHERE email = ?";
    private final String SQL_AUTHENTICATE_USER =
    ...
    public boolean insert(User user){
        ...
        long dbResult = db.insert(DBOpenHelper.TABLE_USER, null, values);
    ...
    public boolean update(User user){
        ...
        long dbResult = db.update(DBOpenHelper.TABLE_USER, values, "email = ?", new String[] { user.getEmail() });
    ...
    public boolean delete(String login){
        try (SQLiteDatabase db = dbHelper.getWritableDatabase()) {
            long dbResult = db.delete(DBOpenHelper.TABLE_USER, "email = ?", new String[]{ login });
